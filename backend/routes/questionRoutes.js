...
const { generateAndFormatSatQuestions } = require('../services/geminiService'); // FIXED: Corrected import path
...
router.post('/add', async (req, res) => {
    const { subject, type, questionText, options, correctAnswer, explanation, difficulty, isMultipleChoice, passageId } = req.body; // Added passageId
    ...
    try {
        const result = await addSatQuestion(
            { subject, type, questionText, options, correctAnswer, explanation, difficulty, isMultipleChoice, passageId }, // Pass passageId
...
router.get('/fetch', async (req, res) => {
    const { subject, count = '1', difficulty, type, passageId } = req.query; // Added passageId
    ...
    try {
        const result = await fetchSatQuestions(subject, count, difficulty, type, passageId); // Pass passageId
 ...
 * Body: { subject, count, difficulty, type (optional), passageText (optional), passageId (optional) }
 */
router.post('/generate', async (req, res) => {
    const { subject, count, difficulty, type, passageText, passageId } = req.body; // Added passageText, passageId
    ...
    // For reading questions, either passageText or passageId should ideally be provided if specific linking is desired.
    // However, Gemini can also generate general reading questions if neither is present.
    ...
    try {
        ...
        const generatedQuestions = await generateAndFormatSatQuestions(subject, count, difficulty, type, passageText, passageId);
